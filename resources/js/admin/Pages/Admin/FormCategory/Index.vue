        ...
        <div class="container-fluid">
            ...
            <div class="card">
                <div class="card-body">
                                ...
                                <template v-if="collection.length>0">
                                    <template v-for="(item, index) in collection" :key="index">
                                        <tr>
                                            ...
                                            <td class="align-middle">{{ item.templates_count }}</td>
                    ...
                    <Pagination v-if="renderPaginate" :paginate="paginate" :url="route('app.admin.category.index')" :current="page" @items="getData"></Pagination>
                    <div v-if="renderPaginate" class="text-end fs-1 fw-bold text-capitalize py-2">Showing {{ paginate.from ? paginate.from : 0 }} to {{ paginate.to ? paginate.to : 0 }} of {{ paginate.total ? paginate.total : 0 }} records</div>
                </div>
            </div>
        </div>
...
export default {
    ...
    methods: {
        ...
        async getData(page = this.page) {
            ...
            await axios.get(route('api.category.get.all'), {
                params: params,
                headers: {
        ...
        async updateItemStatus(event, item, status) {
            ...
            await axios.post(route('api.category.update.status', [item.id]), {
                status: status 
            }, {
                headers: {
                    "Content-Type": "application/json",
        ...
        async delete(ele, item) {
            ...
            await axios.post(route('api.category.delete', [item.id]), {}, {
                headers: {
                    "Content-Type": "application/json",
                    Authorization: "Bearer " + this.token,
                },
        ...
        async blukDelete() {
            ...
            await axios.post(route('api.category.bulk.delete'), {
