    ...
    <AppLayout ref="app_layout" :loader="loader">
        <div class="container-fluid">
            ...
            <div class="row">
                <div class="col-md-6">
                    <template v-if="Object.keys(current_subscription).length>0">
                        <template v-if="customer?.other?.has_exceeded_leads_limit">
                        ...
                        <div class="rounded border border-2 p-4 mb-4">
                            ...
                            <div class="border p-4 rounded d-flex flex-column gap-2 mb-6">
                                ...
                                <div class="d-flex align-items-center gap-2" :class="{
                                    'text-danger': (customer?.other?.has_exceeded_leads_limit)
                                }">
                            ...
                            <template v-if="customer.customer_details?.auto_renewal_subscription">
                            ...
                            <template v-if="!customer.customer_details?.auto_renewal_subscription || current_subscription.package?.free_plan">
                                <template v-if="current_subscription.status != 'paused'">
                                    <div class="d-flex flex-column gap-1 mb-6">
                    ...
                    <template v-else>
                        <div class="rounded border border-2 p-4 mb-4">
                            <div class="d-flex justify-content-between align-items-center gap-3 mb-4">
                                ...
                                <div class="border border-2 rounded py-1 px-2 d-flex align-items-center gap-2">
                                    <span style="width: 7px;height: 7px;" class="rounded bg-info"></span>
                    ...
                    <div class="rounded border border-2 p-4 mb-4" v-if="Object.keys(current_subscription).length>0 && !current_subscription.package?.free_plan && !current_subscription.package?.duration_lifetime">
                        ...
                        <div class="d-flex flex-column align-items-start gap-1 mb-6">
                            ...
                            <div class="border border-2 rounded py-2 px-3 d-flex align-items-center gap-2">
                                <span style="width: 7px;height: 7px;" class="rounded" :class="{
                                    'bg-success': customer.customer_details?.auto_renewal_subscription,
                                    'bg-danger': !customer.customer_details?.auto_renewal_subscription
                                }"></span>
                                <span class="fs-3 fw-bold">{{ customer.customer_details?.auto_renewal_subscription ? 'Enabled' : 'Disabled' }}</span>
                            </div>
                        </div>
                        <div class="d-flex align-items-center gap-2 mt-5">
                            <button class="btn" :class="{
                                'btn-danger': customer.customer_details?.auto_renewal_subscription,
                                'btn-primary': !customer.customer_details?.auto_renewal_subscription
                            }" @click.prevent="confirmAutoRenewalSubscription">{{ customer.customer_details?.auto_renewal_subscription ? 'Disabled' : 'Enabled' }} Auto-Renewal</button>
                        </div>
                    </div>
                </div>
                <div class="col-md-6">
                ...
                <div class="modal-body p-4">
                    <template v-if="user_cards.length>0">
                        <template v-for="(user_card, index) in user_cards" :key="index">
                            ...
                            <template v-else>
                                <div class="rounded bg-light mb-4 p-4">
                                    <div class="d-flex align-items-center gap-2 mb-3">
                                        <div class="d-flex justify-content-between align-items-center gap-3 w-100">
                                            <h3 class="fs-4 fw-bolder mb-1">{{ user_card.card_holder_name }}</h3>
                                            <div class="border rounded bg-primary text-white fw-bolder py-2 px-3 d-flex align-items-center gap-2">
                            ...
                            <template v-if="packages.length>0">
                                <div class="row mb-5 gy-3 gx-3 justify-content-start">
                                    <template v-for="(item, index) in packages" :key="item.id">
                                        <div class="col-md-4" v-if="!(customer.customer_details?.is_avail_free_plan == 1 && item.free_plan)">
                                            <div class="rounded-1 border border-2 p-4 h-100" :class="{
                                                'bg-light': Object.keys(pack).length>0 && pack.id == item.id,
                                                'bg-light opacity-50': isActivePackage(item)
                                            }" @click.prevent="packageSelectIfApplicable(item)">
                                                <span v-if="item.is_private" class="fw-bolder text-warning mb-1 d-inline-block text-uppercase fs-2 rounded-1 mb-3">Hidden</span>
                                                ...
                                                <div class="fs-2 fw-bolder text-dark mb-3" v-if="item.trial_period_days && customer.customer_details?.is_avail_trial == 0">{{ item.trial_period_days }} {{ item.trial_period_days > 1 ? 'Days' : 'Day' }} Free Trial</div>
                                ...
                                <div class="row justify-content-center mb-4" v-if="!(packages.length==1 && isActivePackage(packages[0]))">
                                    <div class="col-md-5">
                                        <div class="d-flex flex-column gap-2">
                                            <button class="btn btn-primary text-uppercase fw-bold rounded-1 fs-2 w-100" @click.prevent="goToPayment()">{{ (Object.keys(pack).length>0) ? 'Countinue' : 'Select a Plan' }}</button>
                                            <button class="btn btn-info text-uppercase fw-bold rounded-1 fs-2 w-100" @click.prevent="sendPaymentLink()" v-if="Object.keys(pack).length>0 && !Object.keys(current_subscription).length">Send Payment Link</button>
                                        </div>
                                    </div>
                                </div>
                            </template>
                ...
                <div class="modal-body p-4">
                    ...
                    <div class="form-group mb-5">
                        <div class="d-flex align-items-center justify-content-between">
                            ...
                            <button class="btn btn-dark btn-sm" @click.prevent="openAddNewWebsiteForm">Add New Website</button>
                        </div>
                    ...
                    <div class="d-flex flex-column gap-3 mt-5">
                        <div class="d-flex flex-column gap-1">
                            ...
                            <div class="text-muted fw-bold fs-3">
                                ...
                                <template v-if="pack.trial_period_days && customer.customer_details?.is_avail_trial == 0">
                        ...
                        <div>
                            <button class="btn w-100 fs-3 fw-bolder text-dark outline-none border-none" @click.prevent="billingPaymentClose"><i class="ti ti-arrow-left"></i> Go Back</button>
                        </div>
                    </div>
                </div>
            </div>
...
export default {
    ...
    methods: {
        ...
        packageSelectIfApplicable(item) {
            if ((this.customer.customer_details?.is_avail_free_plan == 1 && item.free_plan) || this.isActivePackage(item)) {
                return;
            }
        ...
        async getCustomer() {
            ...
            await axios.get(route('api.customer.get.single', [this.$page.props.id]), {
                ...
                if(Object.keys(this.customer).length>0 && this.customer.status != 'active') {
        ...
        async addCard() {
            await axios.post(route('api.subscription.card.add'), {
                ...
                document.body.classList.remove('overflow-hidden');
                this.$inertia.visit(route(route().current(), [this.$page.props.id]));
                this.loader = false;
            }).catch((error) => {
                this.loader = false;
        ...
        confirmAutoRenewalSubscription(event) {
            ...
            this.errors = {};
            let newValue = this.customer.customer_details?.auto_renewal_subscription ? 'disabled' : 'enabled';
            let confirm_button_class = this.customer.customer_details?.auto_renewal_subscription ? 'btn-danger' : 'btn-info';
        ...
        async sendPaymentLink() {
            this.errors = {};
            this.loader = true;
            await axios.post(route('api.subscription.send_payment_link', [this.pack.id]), {
                user_id: this.$page.props.id
            }, {
                headers: {
                    "Content-Type": "application/json",
