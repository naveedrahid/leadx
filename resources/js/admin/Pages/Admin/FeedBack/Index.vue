    ...
    <AppLayout ref="app_layout" :loader="loader">
        <div class="container-fluid">
            <Breadcrumb>
                ...
                <li class="breadcrumb-item text-muted" aria-current="page">Feedback</li>
            </Breadcrumb>
            <div class="mb-3">
                <div class="d-flex align-items-center justify-content-end gap-2">
                    <Link :href="route('app.admin.feedback.index')" class="btn btn-dark btn-sm"><i class="ti ti-refresh"></i> Reload</Link>
                </div>
            </div>
            <div class="card">
                <div class="card-body">
                                        ...
                                        <tr>
                                            ...
                                            <td class="align-middle">
                                                <div class="action-btn d-flex align-items-center gap-2">
                                                    <Link :href="route('app.admin.feedback.show', [item.id])" class="btn btn-info btn-icon btn-sm">
                    ...
                    <Pagination v-if="renderPaginate" :paginate="paginate" :url="route('app.admin.feedback.index')" :current="page" @items="getData"></Pagination>
                    <div v-if="renderPaginate" class="text-end fs-1 fw-bold text-capitalize py-2">Showing {{ paginate.from ? paginate.from : 0 }} to {{ paginate.to ? paginate.to : 0 }} of {{ paginate.total ? paginate.total : 0 }} records</div>
                </div>
            </div>
        </div>
...
export default {
    ...
    methods: {
        ...
        async getData(page = this.page) {
            ...
            await axios.get(route('api.feedback.get.all'), {
                params: params,
                headers: {
        ...
        async delete(ele, item) {
            ...
            await axios.post(route('api.feedback.delete', [item.id]), {}, {
                headers: {
                    "Content-Type": "application/json",
                    Authorization: "Bearer " + this.token,
                },
        ...
        async blukDelete() {
            ...
            await axios.post(route('api.feedback.bulk.delete'), {
