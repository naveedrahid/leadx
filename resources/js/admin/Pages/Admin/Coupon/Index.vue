<template>
    ...
    <AppLayout ref="app_layout" :loader="loader">
        <div class="container-fluid">
            <Breadcrumb>
                <template v-slot:title>Coupons</template>
                <li class="breadcrumb-item text-muted" aria-current="page">Coupons</li>
            </Breadcrumb>
            <div class="mb-3">
                <div class="d-flex align-items-center justify-content-end gap-2">
                    <Link :href="route('app.admin.coupon.create')" class="btn btn-primary btn-sm"><i class="ti ti-plus"></i> Add New</Link>
                    <Link :href="route('app.admin.coupon.index')" class="btn btn-dark btn-sm"><i class="ti ti-refresh"></i> Reload</Link>
                </div>
            </div>
            <div class="card">
                <div class="card-body">
                                ...
                                <template v-if="collection.length>0">
                                    <template v-for="(item, index) in collection" :key="item.id">
                                        <tr>
                                            ...
                                            <td class="align-middle text-center">{{ dateFormat(item.expires_at, 'DD.MM.YYYY') }}</td>
                                            ...
                                            <td class="align-middle">
                                                <div class="action-btn d-flex align-items-center gap-2">
                                                    <Link :href="route('app.admin.coupon.show', [item.id])" class="btn btn-info btn-icon btn-sm">
                                                        <i class="fs-4 ti ti-eye"></i> Show
                                                    </Link>
                                                    <Link :href="route('app.admin.coupon.edit', [item.id])" class="btn btn-primary btn-icon btn-sm">
                    ...
                    <Pagination v-if="renderPaginate" :paginate="paginate" :url="route('app.admin.coupon.index')" :current="page" @items="getData"></Pagination>
                    <div v-if="renderPaginate" class="text-end fs-1 fw-bold text-capitalize py-2">Showing {{ paginate.from ? paginate.from : 0 }} to {{ paginate.to ? paginate.to : 0 }} of {{ paginate.total ? paginate.total : 0 }} records</div>
                </div>
            </div>
        </div>
...
export default {
    ...
    methods: {
        ...
        async getData(page = this.page) {
            ...
            await axios.get(route('api.coupon.get.all'), {
                params: params,
                headers: {
        ...
        async updateItemStatus(event, item, status) {
            ...
            await axios.post(route('api.coupon.update.status', [item.id]), {
                status: status 
            }, {
                headers: {
                    "Content-Type": "application/json",
        ...
        async blukDelete() {
            ...
            await axios.post(route('api.coupon.bulk.delete'), {
