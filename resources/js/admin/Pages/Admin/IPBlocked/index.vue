<template>
    ...
    <AppLayout ref="app_layout" :loader="loader">
        <div class="container-fluid">
            <Breadcrumb>
                <template v-slot:title>IP Blocked</template>
                <li class="breadcrumb-item text-muted" aria-current="page">IP Blocked</li>
            </Breadcrumb>
            <div class="mb-3">
                <div class="d-flex align-items-center justify-content-end gap-2">
                    <Link :href="route('app.admin.customer.create')" class="btn btn-primary btn-sm"><i class="ti ti-plus"></i> Add New</Link>
                    <Link :href="route('app.admin.customer.index')" class="btn btn-dark btn-sm"><i class="ti ti-refresh"></i> Reload</Link>
                </div>
            </div>
            <div class="card">
                <div class="card-body">
                                    ...
                                    <template v-for="(item, index) in collection" :key="item.id">
                                        <tr>
                                            <td class="align-middle">
                                                ...
                                                <template v-else>
                                                    <div class="form-check">
                                                        <input type="checkbox" class="form-check-input bg-light" disabled>
                                                    </div>
                                                </template>
                                            </td>
                                            ...
                                            <td class="align-middle">
                                                <div class="d-flex align-items-center gap-2">
                                                    <template v-if="item.profile_image_url">
                                                        <img :src="item.profile_image_url" :alt="item.fullname" class="rounded-circle" width="35">
                                                    </template>
                                                    <template v-else>
                                                        <span class="text-white d-flex align-items-center justify-content-center text-center rounded-1 fs-2 fw-bold bg-dark" style="width: 40px;height: 40px;">
                                                            {{ item.initials }}
                                                        </span>
                                                    </template>
                                                    <div class="fs-2">{{ item.fullname }}</div>
                                                </div>
                                            </td>
                                            ...
                                            <td class="align-middle">
                                                <template v-if="item.other?.is_subscription_active">
                                                    <span class="text-dark fs-3 fw-bolder">YES</span>
                                                </template>
                                                <template v-else>
                                                    <span class="text-dark fs-3 fw-bolder">NO</span>
                                                </template>
                                            </td>
                                            <td class="align-middle">
                                                <div class="d-flex flex-wrap align-items-center gap-2">
                                                    <Link :href="route('app.admin.customer.subscription', [item.id])" class="btn btn-secondary btn-sm">
                                                        <i class="fs-4 ti ti-coin"></i> Subscriptions
                                                    </Link>
                                                    <Link :href="route('app.admin.customer.show', [item.id])" class="btn btn-info btn-icon btn-sm fs-2">
                                                        <i class="fs-3 ti ti-eye"></i> Show
                                                    </Link>
                                                    <Link :href="route('app.admin.customer.edit', [item.id])" class="btn btn-primary btn-icon btn-sm fs-2">
                                                        <i class="fs-3 ti ti-edit"></i> Edit
                                                    </Link>
                                                    <template v-if="!item.other?.is_subscription_active">
                                                        <button type="button" class="btn btn-danger btn-sm fs-2" @click="deleteItem($event, item)">
                    ...
                    <Pagination v-if="renderPaginate" :paginate="paginate" :url="route('app.admin.customer.index')" :current="page" @items="getData"></Pagination>
                    <div v-if="renderPaginate" class="text-end fs-1 fw-bold text-capitalize py-2">Showing {{ paginate.from ? paginate.from : 0 }} to {{ paginate.to ? paginate.to : 0 }} of {{ paginate.total ? paginate.total : 0 }} records</div>
                </div>
            </div>
        </div>
...
export default {
    ...
    methods: {
        ...
        async getData(page = this.page) {
            ...
            await axios.get(route('api.customer.get.all'), {
                params: params,
                headers: {
        ...
        async updateItemStatus(event, item, status) {
            ...
            await axios.post(route('api.customer.update.status', [item.id]), {
                status: status
            }, {
                headers: {
                    "Content-Type": "application/json",
        ...
        async blukDelete() {
            ...
            await axios.post(route('api.customer.bulk.delete'), {
