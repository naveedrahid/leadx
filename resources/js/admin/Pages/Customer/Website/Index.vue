<template>
    ...
    <AppLayout ref="app_layout" :loader="loader">
        <div class="container-fluid">
            <Breadcrumb>
                <template v-slot:title>Websites</template>
                <li class="breadcrumb-item text-muted" aria-current="page">Websites</li>
            </Breadcrumb>
            <div class="mb-3">
                <div class="d-flex align-items-center justify-content-end gap-2">
                    <Link :href="route('app.customer.website.create')" class="btn btn-primary btn-sm"><i class="ti ti-plus"></i> Add New</Link>
                    <Link :href="route('app.customer.website.index')" class="btn btn-dark btn-sm"><i class="ti ti-refresh"></i> Reload</Link>
                </div>
            </div>
            <div class="card">
                <div class="card-body">
                                ...
                                <template v-if="collection.length>0">
                                    <template v-for="(item, index) in collection" :key="item.id">
                                        <tr>
                                            ...
                                            <td class="align-middle">{{ item.website_name }}</td>
                                            <td class="align-middle">{{ item.website_url }}</td>
                                            ...
                                            <td class="align-middle">
                                                <div class="action-btn d-flex align-items-center gap-2">
                                                    <Link :href="route('app.customer.website.edit', [item.id])" class="btn btn-primary btn-icon btn-sm">
                    ...
                    <Pagination v-if="renderPaginate" :paginate="paginate" :url="route('app.customer.website.index')" :current="page" @items="getData"></Pagination>
                    <div v-if="renderPaginate" class="text-end fs-1 fw-bold text-capitalize py-2">Showing {{ paginate.from ? paginate.from : 0 }} to {{ paginate.to ? paginate.to : 0 }} of {{ paginate.total ? paginate.total : 0 }} records</div>
                </div>
            </div>
        </div>
...
export default {
    ...
    methods: {
        ...
        async updateItemStatus(event, item, status) {
            ...
            await axios.post(route('api.website.update.status', [item.id]), {
                status: status 
            }, {
                headers: {
                    "Content-Type": "application/json",
        ...
        async delete(ele, item) {
            ...
            await axios.post(route('api.website.delete', [item.id]), {}, {
                headers: {
                    "Content-Type": "application/json",
                    Authorization: "Bearer " + this.token,
                },
        ...
        async blukDelete() {
            ...
            await axios.post(route('api.website.bulk.delete'), {
