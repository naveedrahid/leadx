        ...
        <div class="container-fluid">
            <Breadcrumb>
                ...
                <li class="breadcrumb-item">
                    <Link :href="route('app.customer.subscription.index')" class="text-dark">Subscription</Link>
                </li>
                <li class="breadcrumb-item text-muted" aria-current="page">Billing</li>
            </Breadcrumb>
            <div class="mb-4">
                <div class="d-flex align-items-center justify-content-end gap-2">
                    <Link :href="route('app.customer.subscription.index')" class="btn btn-primary btn-sm">All
                    Subscriptions</Link>
                    <Link :href="route('app.customer.subscription.billing.history')" class="btn btn-primary btn-sm">
                    Billing History
                    </Link>
                    <Link :href="route('app.customer.subscription.license')" class="btn btn-primary btn-sm">Plugin
                    License</Link>
                </div>
            </div>
            <template v-if="Object.keys(current_subscription).length > 0">
                ...
                <template v-if="current_subscription.status == 'trialing'">
                    <div class="alert alert-info mb-4">
                        <div class="d-flex align-items-center font-medium me-3 me-md-0">
                            <i class="ti ti-info-octagon fs-7 me-2 flex-shrink-0"></i>
                            <span class="fw-bold">Your subscription is currently in the trial period. Enjoy our services
                                to the fullest!</span>
                        </div>
                    </div>
                </template>
                <template v-if="current_subscription.status == 'paused'">
                    <div class="alert alert-warning mb-4">
                        <div class="d-flex align-items-center font-medium me-3 me-md-0">
                            <i class="ti ti-info-octagon fs-7 me-2 flex-shrink-0"></i>
                            <span class="fw-bold">Your subscription is currently paused. Please resume your subscription
                                to continue enjoying our services. You may also upgrade your plan.</span>
                        </div>
                    </div>
                </template>
                <template v-if="current_subscription.status == 'past_due'">
                    <div class="alert alert-danger mb-4">
                        <div class="d-flex align-items-center font-medium me-3 me-md-0">
                            <i class="ti ti-info-octagon fs-7 me-2 flex-shrink-0"></i>
                            <span class="fw-bold">Your subscription payment is past due. Please change your payment
                                method to settle the outstanding balance and maintain your subscription. You may also
                ...
                <template v-if="current_subscription.status == 'unpaid'">
                    <div class="alert alert-danger mb-4">
                        <div class="d-flex align-items-center font-medium me-3 me-md-0">
                            <i class="ti ti-info-octagon fs-7 me-2 flex-shrink-0"></i>
                            <span class="fw-bold">Your subscription payment is overdue. Please change your payment
                                method to avoid service disruption. You may also upgrade your plan.</span>
                        </div>
                    </div>
                </template>
            </template>
                ...
                <template v-if="Object.keys(subscription).length > 0">
                    <template v-if="subscription.status == 'canceled'">
                        <div class="alert alert-danger mb-4">
                            <div class="d-flex align-items-center font-medium me-3 me-md-0">
                                <i class="ti ti-info-octagon fs-7 me-2 flex-shrink-0"></i>
                                <span class="fw-bold">Your subscription has been canceled. We're sorry to see you go. If
                                    you have any feedback or wish to rejoin, please contact us. You may also start a new
                    ...
                    <template v-if="subscription.status == 'incomplete'">
                        <div class="alert alert-danger mb-4">
                            <div class="d-flex align-items-center font-medium me-3 me-md-0">
                                <i class="ti ti-info-octagon fs-7 me-2 flex-shrink-0"></i>
                                <span class="fw-bold">Your subscription process is incomplete. Please start a new
                                    subscription process.</span>
                            </div>
                        </div>
                    </template>
                    <template v-if="subscription.status == 'incomplete_expired'">
                        <div class="alert alert-danger mb-4">
                            <div class="d-flex align-items-center font-medium me-3 me-md-0">
                                <i class="ti ti-info-octagon fs-7 me-2 flex-shrink-0"></i>
                                <span class="fw-bold">Your subscription attempt has expired. Please start a new
                                    subscription process.</span>
                            </div>
                        </div>
                    </template>
            ...
            <div class="row">
                <div class="col-md-6">
                        ...
                        <div class="rounded border border-2 p-4 mb-4">
                            <div class="d-flex justify-content-between align-items-center gap-3 mb-4">
                                ...
                                <div class="border border-2 rounded py-1 px-2 d-flex align-items-center gap-2">
                                    ...
                                    <span class="fs-2 fw-bold">{{
                                        subsStatuses.hasOwnProperty(current_subscription.status) ?
                                            subsStatuses[current_subscription.status] : current_subscription.status
                                    }}</span>
                                </div>
                            </div>
                            <div class="mb-4">
                                <div class="text-muted fs-6 fw-bold">{{ priceFormat(current_subscription.amount) }} {{
                                    $page.props.currency_code }}</div>
                                <div class="mt-2 fs-2 text-dark fw-bold"
                                    v-if="current_subscription.coupon != null && current_subscription.coupon_expire_at != null">
                                    <template v-if="checkExpiry(current_subscription.coupon_expire_at)">
                                        Total amount with discount: {{ $page.props.currency_symbol }}{{
                                            discountPrice(current_subscription.amount, current_subscription.coupon.amount,
                                                current_subscription.coupon.type) }}
                                    </template>
                                </div>
                            </div>
                            <template v-if="current_subscription.status == 'trialing'">
                                <div class="d-flex flex-column gap-1 mb-6">
                                    ...
                                    <div class="fs-2 fw-bold">
                                        {{ current_subscription.trial_start_at ?
                                            dateFormat(current_subscription.trial_start_at, 'DD.MM.YYYY') : '-' }} to {{
                                            current_subscription.next_billing_date ?
                                                dateFormat(current_subscription.next_billing_date, 'DD.MM.YYYY') : '-' }}
                                        <span class="ps-1">({{ current_subscription.package?.trial_period_days }} {{
                                            current_subscription.package?.trial_period_days > 1 ? 'Days' : 'Day' }} Free
                                            Trial)</span>
                                    </div>
                                </div>
                            </template>
                            <div class="border p-4 rounded d-flex flex-column gap-2 mb-6">
                                <div class="d-flex align-items-center gap-2">
                                    ...
                                    <span class="fs-2 fw-bold">{{ current_subscription.package?.website_limit ?
                                        (numFormat(current_subscription.websites.length) + ' / ' +
                                            numFormat(current_subscription.package?.website_limit)) : 'Unlimited' }}</span>
                                </div>
                                <div class="d-flex align-items-center gap-2" :class="{
                                    ...
                                    <span class="fs-2 fw-bold">{{ current_subscription.package?.lead_limit ?
                                        (current_subscription.leads
                                            + ' / ' + current_subscription.package?.lead_limit) : 'Unlimited' }}</span>
                                </div>
                                <div class="d-flex align-items-center gap-2">
                                    <span class="fs-2 fw-bolder">App Access:</span>
                                    <span class="fs-2 fw-bold">{{ current_subscription.package?.app_access ? 'Yes' :
                                        'No' }}</span>
                                </div>
                            </div>
                            ...
                            <div class="d-flex flex-column gap-1 mb-6">
                                ...
                                <div class="fs-2 fw-bold">{{ current_subscription.start_at ?
                                    dateFormat(current_subscription.start_at,
                                        'DD.MM.YYYY') : '-' }}</div>
                            </div>
                            ...
                            <template v-if="current_subscription.status == 'paused'">
                                <div class="d-flex flex-column gap-1 mb-6">
                                    ...
                                    <div class="fs-2 fw-bold">{{ current_subscription.paused_at ?
                                        dateFormat(current_subscription.paused_at, 'DD.MM.YYYY') : '-' }}</div>
                                </div>
                            </template>

                            <template v-if="user.customer_details?.auto_renewal_subscription">
                                <template
                                    v-if="current_subscription.status != 'paused' && !current_subscription.package?.free_plan">
                                    <div class="d-flex flex-column gap-1 mb-6">
                                        <span class="fs-3 fw-bolder">Next Billing Date</span>
                                        <div class="fs-2 fw-bold">{{ current_subscription.next_billing_date ?
                                            dateFormat(current_subscription.next_billing_date, 'DD.MM.YYYY') : '-' }}
                                        </div>
                                    </div>
                                </template>
                            ...
                            <template
                                v-if="!user.customer_details?.auto_renewal_subscription || current_subscription.package?.free_plan">
                                <template v-if="current_subscription.status != 'paused'">
                                    <div class="d-flex flex-column gap-1 mb-6">
                                        <span class="fs-3 fw-bolder">Ended Date</span>
                                        <div class="fs-2 fw-bold">{{ current_subscription.ended_at ?
                                            dateFormat(current_subscription.ended_at, 'DD.MM.YYYY') : '-' }}</div>
                                    </div>
                                </template>
                            </template>
                            ...
                            <template
                                ...
                                <template v-if="!isExpired(current_subscription.coupon_expire_at)">
                                    <div class="d-flex flex-column gap-1 mb-6">
                                        ...
                                        <span class="fs-2 fw-bold">Coupon: {{ current_subscription.coupon.title
                                            }}</span>
                                        <div class="fs-2 fw-bold">Discount: {{ current_subscription.coupon.discount }}
                                        </div>
                                        <div class="fs-2 fw-bold">Expire at: {{
                                            dateFormat(current_subscription.coupon_expire_at, 'DD.MM.YYYY') }}</div>
                                    </div>
                                </template>
                            </template>
                            ...
                            <div class="d-flex align-items-center gap-2 mt-5">
                                <button class="btn btn-primary"
                                    v-if="current_subscription.status != 'unpaid' && current_subscription.status != 'past_due'"
                                    @click.prevent="takePlan(true)">Upgrade Subscription</button>
                                <button class="btn btn-info" v-if="current_subscription.status == 'paused'"
                                    @click.prevent="confirmResumeSubscription">Resume Subscription</button>
                                <button class="btn btn-danger"
                                    @click.prevent="confirmCancelSubscription">Cancel</button>
                            </div>
                        </div>

                        <template
                            v-if="(current_subscription.status == 'active' || current_subscription.status == 'trialing') && !current_subscription.package?.free_plan && !current_subscription.package?.duration_lifetime">
                            <div class="rounded border border-2 p-4 mb-4">
                                <div class="card-body p-8">
                                    <div class="mb-4">
                                        <div class="fs-5 fw-bolder text-dark mb-1">Discount Code <span
                                                class="text-danger">*</span></div>
                                        <div class="fs-3 text-dark">You can apply coupon in your current plan</div>
                                    </div>
                                    <div class="d-flex flex-column gap-3 align-items-start">
                                        <div class="w-100">
                                            <input type="text" v-model="coupon.discount_code" id="payment-discount-code"
                                                class="form-control py-3" placeholder="e.g. EX123_TEST_CODE">
                                            <div class="text-danger" v-if="errors.discount_code">
                                                {{ errors.discount_code }}
                                            </div>
                                        </div>
                                        <button type="button" class="btn btn-primary rounded"
                                            @click.prevent="applyDiscount">Apply Discount</button>
                                    </div>
                                </div>
                            </div>
                        </template>
                        ...
                        <div class="rounded border border-2 p-4 mb-4">
                            <div class="d-flex justify-content-between align-items-center gap-3 mb-4">
                                ...
                                <div class="border border-2 rounded py-1 px-2 d-flex align-items-center gap-2">
                                    ...
                                    <span class="fs-2 fw-bold">{{ subsStatuses.hasOwnProperty(subscription.status) ?
                                        subsStatuses[subscription.status] : subscription.status }}</span>
                                </div>
                            </div>
                            <div class="mb-4">
                                <div class="text-muted fs-6 fw-bold">{{ priceFormat(subscription.amount) }} {{
                                    $page.props.currency_code }}</div>
                                <div class="mt-2 fs-2 text-dark fw-bold"
                                    v-if="subscription.coupon != null && subscription.coupon_expire_at != null">
                                    <template v-if="checkExpiry(subscription.coupon_expire_at)">
                                        Total amount with discount: {{ $page.props.currency_symbol }}{{
                                            discountPrice(subscription.amount, subscription.coupon.amount,
                            ...
                            <div class="d-flex flex-column gap-1 mb-6">
                                <span class="fs-3 fw-bolder">Started Date</span>
                                <div class="fs-2 fw-bold">{{ subscription.start_at ? dateFormat(subscription.start_at,
                                    'DD.MM.YYYY') : '-' }}</div>
                            </div>
                            <div class="d-flex flex-column gap-1 mb-6">
                                <span class="fs-3 fw-bolder">Ended Date</span>
                                <div class="fs-2 fw-bold">{{ subscription.ended_at ? dateFormat(subscription.ended_at,
                                    'DD.MM.YYYY') : '-' }}</div>
                            </div>
                            <div class="d-flex align-items-center gap-2 mt-5">
                    ...
                    <div class="rounded border border-2 p-4 mb-4"
                        v-if="!current_subscription.package?.free_plan && !current_subscription.package?.duration_lifetime">
                        ...
                        <div class="d-flex flex-column align-items-start gap-1 mb-6">
                            ...
                            <div class="border border-2 rounded py-2 px-3 d-flex align-items-center gap-2">
                                ...
                                <span class="fs-3 fw-bold">{{ user.customer_details?.auto_renewal_subscription ?
                                    'Enabled' : 'Disabled'
                                    }}</span>
                            </div>
                        </div>
                        <div class="d-flex align-items-center gap-2 mt-5">
                            ...
                            }" @click.prevent="confirmAutoRenewalSubscription">{{
                                user.customer_details?.auto_renewal_subscription ? 'Disabled' : 'Enabled' }}
                                Auto-Renewal</button>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="rounded border border-2 p-4 mb-4" v-if="Object.keys(current_subscription).length > 0">
                        <div class="d-flex justify-content-between align-items-start gap-3 mb-4">
                            ...
                            <span class="d-block fw-bold fs-2 mb-1 text-end">
                                ...
                                <span class="text-dark fw-bolder">{{ current_subscription.package?.website_limit ?
                                    numFormat(current_subscription.package?.website_limit) : 'Unlimited' }}</span>
                            </span>
                        </div>
                        ...
                        <div class="d-flex align-items-center gap-2">
                            ...
                            <button class="btn btn-primary" @click.prevent="openChangeWebsiteForm">Select
                                Websites</button>
                        </div>
                    </div>
                    <template v-if="Object.keys(default_card).length > 0">
                        <div class="rounded border border-2 p-4 mb-4">
                            <div class="d-flex justify-content-between align-items-center gap-3 mb-5">
                                ...
                                <button class="btn btn-primary btn-sm" @click.prevent="addNewCardForm">Add New
                                    Card</button>
                            </div>
                            <div class="d-flex flex-column gap-1 mb-5">
                                ...
                                <div class="fs-2">Card expires at {{ numFormat(default_card.exp_month) }}/{{
                                    default_card.exp_year }}</div>
                            </div>
                            <div class="d-flex align-items-center gap-2">
                                <button class="btn btn-info" @click.prevent="changePaymentMethod">Change Payment
                                    Method</button>
                                <button class="btn btn-danger"
                                    @click.prevent="confirmRemovePaymentMethod($event, default_card.id)"
                                    v-if="user_cards.length > 1">Remove Card</button>
                            </div>
                        </div>
                    </template>
                    <template v-else>
                        <div class="rounded border border-2 p-4 mb-4">
                            ...
                            <div class="d-flex flex-column gap-1 mb-5">
                                <div class="fs-3">You don't have a default card set. Please add a new card or set a
                                    default card to proceed with your subscription.</div>
                            </div>
                            <div class="d-flex align-items-center gap-4">
                                <button class="btn btn-info" @click.prevent="changePaymentMethod"
                                    v-if="user_cards.length > 0">Select Payment Method</button>
                            </div>
                        </div>
                    </template>
                </div>
        ...
        <div class="modal-dialog modal-dialog-scrollable modal-lg">
            <div class="modal-content">
                <div class="modal-header p-4">
                    ...
                    <button type="button" class="btn p-0 outline-none border-none"
                        @click.prevent="closePaymentMethodList">
                        ...
                        <template v-for="(user_card, index) in user_cards" :key="index">
                            <template v-if="!user_card.is_default">
                                <div class="pmcards-items">
                                    <input type="checkbox" class="d-none" :id="user_card.id + '-payment-method'"
                                        :checked="form.change_payment_method.paymentMethodId == user_card.pm_id">
                                    <label class="rounded border border-2 mb-4 p-4 w-100" :class="{
                                        ...
                                        <div class="d-flex flex-column gap-1 w-100">
                                            <div class="d-flex align-items-center gap-2">
                                                <span class="fs-3 text-capitalize fw-bold mb-1">{{ user_card.brand
                                                    }}</span>
                                                <div class="fs-2">**** **** **** {{ user_card.last4 }}</div>
                                            </div>
                                            <div class="fs-2">Card expires at {{ numFormat(user_card.exp_month) }}/{{
                                                user_card.exp_year }}</div>
                                        </div>
                                    </label>
                                </div>
                            </template>
                                ...
                                <div class="rounded border border-2 border-dashed border-info mb-4 p-4">
                                    <div class="d-flex align-items-center gap-2 mb-3">
                                        <div class="d-flex justify-content-between align-items-center gap-3 w-100">
                                            <h3 class="fs-4 fw-bolder mb-1">{{ user_card.card_holder_name }}</h3>
                                            <div
                                                class="border rounded bg-light-info text-info fw-bolder py-2 px-3 d-flex align-items-center gap-2">
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-primary" @click.prevent="setToDefaultPaymentMethod"
                        :disabled="form.change_payment_method.paymentMethodId == ''">Set to Default</button>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="modal-dialog modal-dialog-scrollable modal-lg">
            <div class="modal-content">
                <div class="modal-header p-4">
                    ...
                    <button type="button" class="btn p-0 outline-none border-none"
                        @click.prevent="confirmDiscardNewCardForm">
                        <i class="ti ti-x fs-7"></i>
                    </button>
                </div>
                <div class="modal-body p-4">
                    <div class="row">
                        <div class="col-md-12">
                            <div class="form-group mb-4">
                                <label class="form-label mb-1" for="new-card-holder-name">Name On Card <span
                                        class="text-danger">*</span></label>
                                <input type="text" v-model="form.add_new_card.card_holder_name"
                                    id="new-card-holder-name" class="form-control py-3" placeholder="e.g. John Doe">
                        ...
                        <div class="col-md-12">
                            <div class="form-group mb-4">
                                <label class="form-label mb-1" for="new-card-element">Card Information <span
                                        class="text-danger">*</span></label>
                        ...
                        <div class="col-md-12">
                            <div class="form-group mb-4">
                                <div class="form-check mb-0 d-flex align-items-center gap-2">
                                    <input type="checkbox" v-model="form.add_new_card.set_to_default"
                                        id="new-set_to_default" class="form-check-input">
                                    <label class="form-check-label fs-3 fw-bold text-muted" for="new-set_to_default">Set
                                        the
                                        card to default for further billing.</label>
                                </div>
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-danger"
                        @click.prevent="confirmDiscardNewCardForm">Discard</button>
        ...
        <div class="modal-dialog modal-dialog-scrollable">
            <div class="modal-content">
                <div class="modal-header p-4">
                    ...
                    <button type="button" class="btn p-0 outline-none border-none"
                        @click.prevent="closeAddNewWebsiteForm">
                        <i class="ti ti-x fs-7"></i>
                    </button>
                </div>
                <div class="modal-body p-4">
                        ...
                        <div class="col-md-12">
                            <div class="form-group mb-4">
                                ...
                                <input type="text" id="website_name" class="form-control"
                                    v-model="website_form.website_name">
                        ...
                        <div class="col-md-12">
                            <div class="form-group mb-4">
                                ...
                                <input type="url" id="website_url" class="form-control"
                                    v-model="website_form.website_url">
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header p-4">
                    ...
                    <button type="button" class="btn p-0 outline-none border-none"
                        @click.prevent="closeChangeWebsiteForm">
                        <i class="ti ti-x fs-7"></i>
                    </button>
                </div>
                <div class="modal-body p-4">
                    <div class="d-flex flex-column gap-1 mb-2">
                        ...
                        <p class="mb-0 fs-2 fw-bold text-muted">You can select {{
                            !current_subscription.package?.website_limit ? 'unlimited' :
                                numFormat(current_subscription.package?.website_limit) }} website</p>
                    </div>
                    <div class="form-group mb-5">
                        <template v-if="Object.keys(current_subscription).length > 0">
                            <Multiselect v-model="selectedWebsite" :options="websites_options"
                                :max="!current_subscription.package?.website_limit ? -1 : parseInt(current_subscription.package?.website_limit)"
                                mode="tags" placeholder="Select Your Websites">
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-primary" @click.prevent="saveSelectedWebsites">Save
                        Website</button>
                </div>
            </div>
        </div>
                            ...
                            <template v-if="packages.length > 0">
                                <div class="row mb-5 gy-3 gx-3 justify-content-start">
                                    <template v-for="(item, index) in packages" :key="item.id">
                                        <div class="col-md-4"
                                            v-if="!(user.customer_details?.is_avail_free_plan == 1 && item.free_plan)">
                                            <div class="rounded-1 border border-2 p-4 h-100" :class="{
                                                ...
                                                <span v-if="item.recommended"
                                                    class="fw-bolder text-primary mb-1 d-inline-block text-uppercase fs-1 rounded-1">Best
                                                    Deal</span>
                                                <div class="mb-3 d-flex flex-column gap-1">
                                                    <span class="fw-bolder text-uppercase fs-3 d-block">{{ item.title
                                                        }}</span>
                                                    <div class="fs-2" v-if="item.description">{{ item.description }}
                                                    </div>
                                                </div>
                                                <div class="mb-3">
                                                    <h2 class="fw-bolder fs-6 mb-0" v-if="item.free_plan">Free</h2>
                                                    <h2 class="fw-bolder fs-6 mb-0" v-else>
                                                        {{ priceFormat(item.price, true, item.strip_precent) }}
                                                        <span class="fs-2 text-capitalize">/ {{ item.duration_lifetime ?
                                                            'Lifetime' : item.format_duration }}</span>
                                                    </h2>
                                                    <div class="fs-3 fw-bold" v-if="item.sale_price !== null">Normally
                                                        <del class="text-dark">{{ priceFormat(item.regular_price)
                                                            }}</del>
                                                    </div>
                                                </div>
                                                <div class="fs-2 fw-bolder text-dark mb-3"
                                                    v-if="item.trial_period_days && user.customer_details?.is_avail_trial == 0">
                                                    {{ item.trial_period_days }} {{ item.trial_period_days > 1 ? 'Days'
                                                        : 'Day' }} Free Trial</div>
                                ...
                                <div class="row justify-content-center mb-4"
                                    v-if="!(packages.length == 1 && isActivePackage(packages[0]))">
                                    <div class="col-md-6">
                                        <button class="btn btn-primary w-100" @click.prevent="goToPayment()">{{
                                            (Object.keys(pack).length > 0)
                                                ? 'Countinue' : 'Select a Plan' }}</button>
                                    </div>
                                </div>
                            </template>
                            <template v-else>
                                <div
                                    class="p-4 rounded my-5 bg-light d-flex justify-content-center flex-column gap-2 border border-2">
                                    <h4 class="fs-4 fw-bolder mb-2">No Plans Found!</h4>
                                    <p class="fs-2 mb-0">Please kindly contact the administrator for further assistance.
                                    </p>
                                    <p class="fs-2 mb-0">For any query please call us at <a
                                            :href="'tel:' + $page.props.app.support_number" class="text-dark fw-bold">{{
                                                $page.props.app.support_number }}</a> or send an email to <a
                                            :href="'mailto:' + $page.props.app.support_mail"
                                            class="text-dark fw-bold">{{
                                                $page.props.app.support_mail }}</a></p>
                                </div>
                            </template>
                        </div>
                    </div>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body p-4">
                    <div class="mb-5 text-center">
                        ...
                        <div class="text-muted fw-bold fs-3" v-if="!pack.free_plan">Please provide your payment
                            information,
                            including your name, credit card number, expiration date, and CVC code to complete the
                            transaction.</div>
                        <div class="text-muted fw-bold fs-3" v-else>No payment information is required for our free
                            plan.
                        </div>
                    </div>
                    <template v-if="!pack.free_plan">
                        <template v-if="has_default_card === true && use_default_card == true">
                            <div class="mb-4">
                                <button type="button" class="btn btn-dark btn-sm" @click.prevent="paymentAddNewCard">Add
                                    New Card</button>
                            </div>
                            <div class="rounded border border-2 mb-4 p-3 d-flex flex-column gap-2">
                                ...
                                <div class="d-flex align-items-center gap-3">
                                    <div
                                        class="bg-light border border-2 rounded p-6 d-flex align-items-center justify-content-center">
                                        <i class="ti ti-credit-card text-dark d-block fs-7"></i>
                                    </div>
                                    <div class="d-flex flex-column gap-1">
                                        <div class="fs-4 text-capitalize fw-bolder mb-1">{{ default_card.brand }} ****
                                            {{ default_card.last4 }}</div>
                                        <div class="fs-2">Card expires at {{ numFormat(default_card.exp_month) }}/{{
                                            default_card.exp_year }}</div>
                                    </div>
                                </div>
                        ...
                        <template v-if="use_default_card == false">
                            <div class="mb-4" v-if="has_default_card === true">
                                <button type="button" class="btn btn-info btn-sm"
                                    @click.prevent="use_default_card = true">Use Default Card</button>
                            </div>
                            <div class="form-group mb-3">
                                <label class="form-label mb-1" for="new-card-holder-name">Name On Card <span
                                        class="text-danger">*</span></label>
                                <input type="text" v-model="form.payment.card_holder_name" id="new-card-holder-name"
                            ...
                            <div class="form-group mb-3">
                                <label class="form-label mb-1" for="payment-card-element">Card Information <span
                                        class="text-danger">*</span></label>
                        ...
                        <div class="form-group mb-3">
                            ...
                            <input type="text" v-model="form.payment.discount_code" id="payment-discount-code"
                                class="form-control" placeholder="e.g. EX123_TEST_CODE">
                            <div class="text-danger" v-if="errors.discount_code">
                                {{ errors.discount_code }}
                            </div>
                        </div>
                    ...
                    <div class="form-group mb-5">
                        <div class="d-flex align-items-center justify-content-between gap-2 mb-2">
                            <div>
                                ...
                                <p class="mb-1 fs-2 fw-bold text-muted">You can select {{ !pack.website_limit ?
                                    'unlimited'
                                    : numFormat(pack.website_limit) }} website</p>
                            </div>
                            <button class="btn btn-info btn-sm fs-2" @click.prevent="openAddNewWebsiteForm">Add New
                                Website</button>
                        </div>
                        <Multiselect v-model="selectedWebsite" :options="websites_options"
                            :max="!pack.website_limit ? -1 : parseInt(pack.website_limit)" mode="tags"
                            placeholder="Select Your Websites">
                            <template v-slot:option="{ option }">
                                <div>
                                    <span class="d-block">{{ option.label }}</span>
                                    <span class="d-block fs-2 text-muted">{{ option.url }}</span>
                    ...
                    <div class="d-flex flex-column gap-3 mt-5 mb-4">
                        ...
                        <div class="d-flex align-items-center gap-2">
                            <button class="btn btn-primary" @click.prevent="payNow()">Pay Now <strong>{{
                                priceFormat(pack.price) }}</strong></button>
...
export default {
    ...
    methods: {
        hasStrip(p) {
            const pct = this.toNumber(p?.strip_precent ?? p?.strip_percent ?? 0);
            return pct > 0;
        },
        displayPriceWithStrip(p) {
            const price = this.toNumber(p?.price);
            const pct = this.toNumber(p?.strip_precent ?? p?.strip_percent ?? 0);
            if (pct > 0) {
        ...
        // },
        priceFormat(price, symbol = true, stripPercent = 0) {
            let amount = Number(price) || 0;
            const pct = Number(stripPercent) || 0;
            ...
            const out = amount.toFixed(2);
            const sym = this.$page?.props?.currency_symbol ?? '$';
